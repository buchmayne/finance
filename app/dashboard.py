...
def display_summary_metrics(spending_data: list, category_data: list):
    ...
    avg_spending = (
        sum(item["spending"] for item in spending_data) / len(spending_data)
        if spending_data
        else 0
    )
    total_categories = len(category_data) if category_data else 0
    top_category = (
        category_data[0]["category"].replace("_", " ").title()
        if category_data
        else "N/A"
    )
    ...
    with col1:
        st.metric(
            ...
            delta=(
                f"${latest_spending - avg_spending:,.0f}"
    ...
    with col2:
        st.metric(label="Monthly Average", value=f"${avg_spending:,.0f}")
...
def main():
    ...
    category_level = st.sidebar.selectbox(
        ...
        format_func=lambda x: (
            "High-Level Categories" if x == "meta_category" else "Detailed Categories"
        ),
    )

    # Chart type for categories
    category_chart_type = st.sidebar.radio(
        "Category Chart Type", options=["bar", "pie"], index=0
    )
    ...
    with st.spinner("Loading financial data..."):
        ...
        category_response = fetch_api_data(
            ...
            {"period": "last_12_months", "level": category_level},
        )
        ...
        if spending_data:
            ...
            with st.expander("📊 View Raw Spending Data"):
                ...
                if not df_display.empty:
                    # Format for display
                    df_display["spending_formatted"] = df_display["spending"].apply(
                        lambda x: f"${x:,.2f}"
                    )
                    df_display["date"] = (
                        df_display["year"].astype(str)
                        + "-"
                        + df_display["month"].astype(str).str.zfill(2)
                    )
                    display_df = df_display[["date", "spending_formatted"]].rename(
                        columns={"date": "Month", "spending_formatted": "Spending"}
                    )
    ...
    with col2:
        ...
        if category_data:
            ...
            with st.expander("📊 View Raw Category Data"):
                ...
                if not df_display.empty:
                    # Format for display
                    df_display["total_spending_formatted"] = df_display[
                        "total_spending"
                    ].apply(lambda x: f"${x:,.2f}")
                    df_display["category_formatted"] = df_display["category"].apply(
                        lambda x: x.replace("_", " ").title()
                    )
                    display_df = (
                        df_display[["category_formatted", "total_spending_formatted"]]
                        .rename(
                            columns={
