...
class TestNormalizeDescription:
    ...
    def test_normalize_description_converts_to_uppercase(self):
        ...
        df = pd.DataFrame({
            'description': ['test transaction', 'Another Test']
        })
...
class TestNormalizeDate:
    ...
    def test_normalize_date_creates_date_column(self):
        ...
        assert isinstance(result['date'].iloc[0], (pd.Timestamp, pd.DatetimeTZDtype, type(pd.to_datetime('2024-01-15').date())))

    def test_normalize_date_creates_temporal_columns(self):
        """Test that year, month, year_month columns are created."""
        df = pd.DataFrame({
            'date': pd.to_datetime(['2024-01-15', '2024-12-25'])
        })
    ...
    def test_normalize_date_day_of_week_sunday_is_7(self):
        ...
        df = pd.DataFrame({
            'date': pd.to_datetime(['2024-01-14'])
        })
...
class TestCategorizeBankTransactions:
    ...
    def test_categorize_individual_bank_transaction(self, description, expected_category):
...
class TestCategorizeCreditCardTransactions:
    ...
    def test_categorize_individual_credit_card_transaction(self, description, expected_category):
...
class TestStagingDataCleaningPipeline:
    ...
    def test_clean_bank_account_data_applies_all_transformations(self, mock_get_db, temp_db_connection):
        ...
        raw_df.to_sql('raw_bank_account_transactions', temp_db_connection, if_exists='replace', index=False)
    ...
    @patch('etl.layers.staging.get_db')
    def test_clean_credit_card_data_applies_all_transformations(self, mock_get_db, temp_db_connection):
        ...
        # Write to temp database
        raw_df.to_sql('raw_credit_card_transactions', temp_db_connection, if_exists='replace', index=False)
        ...
        assert 'category' in result.columns
        assert result['category'].iloc[0] in ['OVATION', 'OVATION_WEEKDAY', 'OVATION_WEEKEND']
        assert 'year' in result.columns
        assert 'month' in result.columns
...
class TestStagingTableCreation:
    ...
    def test_create_staging_bank_account_transactions_writes_to_db(
        ...
        result = pd.read_sql('SELECT * FROM staging_bank_account_transactions', temp_db_connection)
        assert len(result) == 1
        assert result['category'].iloc[0] == 'SALARY'
    ...
    def test_create_staging_credit_card_transactions_writes_to_db(
        ...
        result = pd.read_sql('SELECT * FROM staging_credit_card_transactions', temp_db_connection)
        assert len(result) == 1
